...
- [[#functions-and-variables][Functions and Variables]]
  ...
  - [[#sorting-the-list][Sorting the list]]
    ...
    - [[#examples-for--rearranging][Examples for  "rearranging":]]
...
| lister-rearrange{-list/this-level/dwim} | Abstract interface for all kind of "rearranging"     |
|-----------------------------------------+------------------------------------------------------|
...
*** Examples for  "rearranging":

#+begin_src emacs-lisp
    (lister-rearrange-list buf 'reverse) ;; reverse the list at point
    (lister-rearrange-list buf 'ignore) ;; delete the list at point
    ...
    (lister-rearrange-list buf 'delete-every-second-item)
  ...
  (let ((filter-fn (apply-partially #'seq-filter (lambda (item) (< 10 (car item))))))
    (lister-rearrange-list (current-buffer) filter-fn))

  ;; sort by predicate function >
  ;; note that we use 'sort-by', which checks against the CAR of the wrapped item
  (lister-rearrange-list (current-buffer) (apply-partially #'seq-sort-by #'car #'>))
#+end_src
...
** Changes to the current version (no new release)
 ...
 + Abstracted "sorting" to "rearranging"
